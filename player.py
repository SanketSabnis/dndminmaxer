...
class Character(Combat):
    ...
    def __init__(self, **params):
        ...
        if not hasattr(self, "attack_att"):  self.attack_att = "str_mod"
        ...
        if hasattr(self, "hit_die"):
            ...
            self.hit_points = evaluate("%s+%s+%s(%s+%s)"%(self.hit_die.replace("d",""), hp, self.con_mod, self.level-1,  self.con_mod))
    ...
    def check_feats(self, hit_chance=1, raw_hit_chance=1.25):
        ...
        if "savage" in self.feats:
            ...
            if self.attacks >1:
                self.dmg_str = "%s*(DropLowest%s%s+%s)+%s(%s*(%s+%s))"% (hit_chance,num_die+1, self.weapon, self.attack_mod+ self.dmg_bonus, self.smite, self.attacks-1, hit_chance, self.weapon, self.str_mod+ self.dmg_bonus)
            else:
                self.dmg_str = "%s*(DropLowest%s%s+%s)"% (hit_chance, num_die+1,self.weapon, self.attack_mod+ self.dmg_bonus)
        if "gwm" in self.feats:
            p_chance = min(1, (raw_hit_chance*20-5)/20.0)
            p_attack = "(%s*(%s+%s+10%s))"% (p_chance, self.weapon, self.attack_mod + self.dmg_bonus, self.smite)
            n_attack = "(%s*(%s+%s%s))"% (hit_chance, self.weapon, self.attack_mod + self.dmg_bonus, self.smite)
        ...
        if "polearm_master" in self.feats or "xbow_expert" in self.feats:
            ...
            if "gwm" in self.feats:
                p_attack ="%s*(%s+%s+10)"% (p_chance, die, self.attack_mod + self.dmg_bonus)
                n_attack = "%s*(%s+%s)"% (hit_chance, die, self.attack_mod + self.dmg_bonus)
                ...
                self.dmg_str = "%s+(%s*(%s+%s%s))"% (self.dmg_str, hit_chance, die, self.attack_mod+ self.dmg_bonus, self.reduced_smite())
    ...
    def check_modifiers(self, hit_chance):
        if hasattr(self, "haste"):
            self.dmg_str += "+(%s*(%s+%s%s))"% (hit_chance, self.weapon, self.attack_mod + self.dmg_bonus, self.reduced_smite())        

    def get_proficiency_bonus(self):
        if not hasattr(self, "level"): return 2
    ...
    def reduced_smite(self):
        ...
        smite_split = plus_split[1].split("d") if len(plus_split) > 1 and len(plus_split[1]) > 2 else [self.smite]
        reduced_smite = "+%sd%s"% (int(smite_split[0])-1, smite_split[1])  if len(smite_split)==2 else self.smite
